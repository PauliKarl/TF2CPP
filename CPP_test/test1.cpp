...
int main( int argc, char ** argv)
{
	...
	file = "G:/data/demo/GF3_MYN_FSI_008230_E139.6_N35.4_20180303_L1A_HHHV_L10003038823_D_HV_pwr_ql__3296_8240.png";

	//输入文件路径分解
	char szDrive[5];   //磁盘名
	char szDir[50];       //路径名
	char szFname[150];   //文件名
	char szExt[10];       //后缀名
...
{	
	...
	for (int i = 0; i < numDetections; ++i)
	{
		...
		if (score > confThreshold)
		{
			...
			//***********Extract the mask for the object: 提取旋转框和角度************//
			// 图像左上角为原点，水平为x, 竖直为y
			// (rcx, rcy): 矩形的中心点
			// rw: 矩形的窄边
			// rh: 矩形的长边
			// angle: 矩形长边与水平方向的夹角，[-PI/2, PI/2], 向下为正
			...
			int rcx =(left + width / 2);
			int rcy =(top + height / 2);
			float angle, rw, rh;
			rw = rRect.size.width;
			...
			// rotation rectangle box 目标位置
			TiXmlElement *roboxElement = new TiXmlElement("robox");
			ObjElement->LinkEndChild(roboxElement);

			// rcx
			TiXmlElement *rcxElement = new TiXmlElement("rcx");
			roboxElement->LinkEndChild(rcxElement);

			char rcxx[10];
			sprintf_s(rcxx, "%d", rcx);
			TiXmlText *rcxContent = new TiXmlText(rcxx);
			rcxElement->LinkEndChild(rcxContent);

			// rcy
			TiXmlElement *rcyElement = new TiXmlElement("rcy");
			roboxElement->LinkEndChild(rcyElement);

			char rcyy[10];
			sprintf_s(rcyy, "%d", rcy);
			TiXmlText *rcyContent = new TiXmlText(rcyy);
			rcyElement->LinkEndChild(rcyContent);

			// rw
			TiXmlElement *rwElement = new TiXmlElement("rw");
			roboxElement->LinkEndChild(rwElement);

			char rww[10];
			sprintf_s(rww, "%f", rw);
			TiXmlText *rwContent = new TiXmlText(rww);
			rwElement->LinkEndChild(rwContent);

			// rh
			TiXmlElement *rhElement = new TiXmlElement("rh");
			roboxElement->LinkEndChild(rhElement);

			char rhh[10];
			sprintf_s(rhh, "%f", rh);
			TiXmlText *rhContent = new TiXmlText(rhh);
			rhElement->LinkEndChild(rhContent);

			// angle
			TiXmlElement *angleElement = new TiXmlElement("angle");
			roboxElement->LinkEndChild(angleElement);

			char rtheta[10];
			sprintf_s(rtheta, "%f", angle);
			TiXmlText *angleContent = new TiXmlText(rtheta);
			angleElement->LinkEndChild(angleContent);
